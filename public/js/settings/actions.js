/* ------------------------------------------------------------------ */
/*  Helper widgets: toast + bootstrap wrappers                        */
/* ------------------------------------------------------------------ */
const toast = (msg, variant = "success") => {
  ...
  /* Delete click */
  document.querySelector("#actionsTable tbody").addEventListener("click", async e => {
  ...
  let deviceCache = [];                // list from /settings/devices/data
  let schemaCache = new Map();         // devId -> schema JSON
  ...
  async function fillDeviceSelects() {
    ...
    ["trigger_device", "result_device"].forEach(name => {
      ...
      devs.filter(d => d.enabled).forEach(d =>
        sel.insertAdjacentHTML("beforeend",
          `<option value="${d.id}">${d.name}</option>`));
    });
  }
  
  /* when device changes → load topics / commands */
  ...
  /* when topic changes → adapt value input */
  function adaptValueInput(prefix) {
    const topicSel = form[`${prefix}_topic`];
    const meta     = topicSel.selectedOptions[0]?.dataset.meta;
    if (!meta) return;
    ...
    const inp = valueCol.querySelector("input");
    ...
    if (m.type === "enum" || m.type === "bool") {
      ...
      /* comparator select + numeric input */
  ...
  document.getElementById("saveActionBtn").addEventListener("click", async () => {
    ...
    if (res.ok) { modal.hide(); await loadActionRows(); toast("Saved"); }
